...
- PR capabilities should be exposed as multiple narrow orchestrator tools, not one broad GitHub client.
- `open_pull_request` later is an orchestrator tool bound to current session/run context.
- `get_pull_request_status` lets the orchestrator inspect the session PR without relying on chat memory.
- `update_pull_request_description` lets the orchestrator edit title/body on the session-owned PR.
- `read_pull_request_comments` lets the orchestrator read issue comments, review comments, file/line context, unresolved state, and comment authors.
- Later optional tools: `reply_to_pull_request_comment` and `mark_pull_request_ready_for_review`, both permission-gated.
- Tool inputs should stay narrow. Examples: title/body/draft for opening; title/body for description edits; filters such as unresolved-only or since timestamp for comments.
- Tools should resolve repo, PR number, branch, installation/account, token, and workspace from `currentRun -> ChatSession -> SessionPullRequest` server-side.
- Token never enters model prompt, sandbox logs, events, or tool args.
- PR tools are available to the orchestrator, not the CodeWorker by default.
- Default permissions: create draft PR, read PR status/checks/comments, edit title/body on a PR created or bound by the current session.
- Explicitly excluded by default: merge PR, close PR, delete branch, force push, repo settings/admin, arbitrary GitHub API access.

Add a durable `SessionPullRequest` binding so later turns can inspect or update the PR without depending on model memory.
...
This is required for real follow-up loops:
